...
def read_histogram(filename, x_label, title, savename, data_date, histo_mean, histo_std):
    ...
    histo_center = histo_sorted[np.where(((histo_mean - histo_std) <= histo_sorted) & (histo_sorted <= (histo_mean + histo_std)))]
    ...
    x_values = np.linspace(popt[1] - 2*popt[2], popt[1] + 2*popt[2], 100000)
