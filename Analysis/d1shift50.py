...
def center_check(Nloops,numhead,data_date):
    ...
    for i in range(Nloops):
        ...
        if index_50 < 400:
            too_small_list = np.append(too_small_list,i)
        else:
    ...
    return(max_index,min_index,center_index,too_small_list)

#shifting to align
def d1shift50(data_date,numhead):
    Nloops = len(os.listdir('G:/data/watchman/'+data_date+'_watchman_spe/d1/d1_baseline_shifted'))
    (max_index,min_index,center_index,too_small_list) = center_check(Nloops,numhead,data_date)
    print(min_index)
    print(center_index)
    print(max_index)
    for i in range(Nloops):
        if (i in too_small_list):
            pass
        else:
            filename = 'G:/data/watchman/'+data_date+'_watchman_spe/d1/d1_baseline_shifted/D1--waveforms--%05d.txt' % i
