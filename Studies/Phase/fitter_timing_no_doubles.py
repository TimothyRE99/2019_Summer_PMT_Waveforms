...
def fitter_timing(datadate,numhead,samplerate,samplerate_name,shaping):
    ...
    ax.set_ylabel('Count')
    ax.set_title('Fitter Timings, Doubles Removed')
    ...
    plt.ylabel('Count')
    plt.title('Chi Squared Counts - Fitter Timings, Doubles Removed')
    plt.get_current_fig_manager().window.showMaximized()
    plt.show()
    plt.close()
    ...
    ax.set_title('Chi Squared vs. Timing Corrections - Fitter Timings, Doubles Removed')
