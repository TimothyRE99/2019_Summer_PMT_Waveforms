#Fitter timing portion using exact waveform
...
def fitter_timing(datadate,samplerate_name,shaping):
    ...
    ax.set_ylabel('Count')
    ax.set_title(samplerate_name+' - Fitter Timings, Exact Waveform')
    ...
    plt.ylabel('Count')
    plt.title(samplerate_name+' - Chi Squared Counts - Fitter Timings, Exact Waveform')
    plt.get_current_fig_manager().window.showMaximized()
    plt.show()
    plt.close()
    ...
    ax.set_title(samplerate_name+' - Chi Squared vs. Timing Corrections - Fitter Timings, Exact Waveform')
